...
-   **🤖 Multiple AI Providers**: Support for Ollama, OpenAI (extensible to more)
-   **💾 Multiple Databases**: PostgreSQL, MySQL, MongoDB (extensible)
-   **🔄 Runtime Configuration**: Choose AI model and database via npm scripts
-   **🏗️ Clean Architecture**: Interface-based design following SOLID principles
-   **⚡ Type-Safe**: Written in TypeScript for better developer experience
-   **🔌 Pluggable**: Easy to add new AI models or databases
-   **📊 Schema-Aware**: Automatically retrieves and uses database schema
-   **🛡️ Error Handling**: Comprehensive error handling and validation
-   **🔍 Query Sanitization**: Extracts clean SQL from AI responses
...
└────────┬────────┘
    ...
    │         │
┌───▼──┐  ┌──▼───┐
│  AI  │  │  DB  │ ← Provider Implementations
└──────┘  └──────┘
    │         │
┌───▼─────────▼──┐
...
│   │   └── IDatabaseProvider.ts      # Database provider interface
│   ├── providers/
│   │   ├── ai/
...
│   │       └── MongoDBProvider.ts    # MongoDB implementation
│   ├── services/
│   │   └── TextToSQLService.ts       # Core business logic
│   ├── controllers/
│   │   └── QueryController.ts        # HTTP request handling
│   ├── config/
